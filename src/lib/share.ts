...
/** Starts a separate browser download for every selected image. */
export function downloadFiles(files: { url: string; filename: string }[]) {
  for (const file of files) downloadFile(file.url, file.filename);
}
